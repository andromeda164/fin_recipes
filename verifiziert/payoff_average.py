""" ******************************************************
    ...
    Literatur-Ref   --
*******************************************************"""
import fin_recipes, math, functools

def add(x,y): return x+y

def payoff_arithmetric_average_call(prices, X):
    sum = float(functools.reduce(add, prices))
...
def payoff_bermudan_arithmetic_average_call(sampling_indexes, prices, X):
    ...
    sum = float(functools.reduce(add, slice))
...
if __name__=='__main__':
    ...
    print('The arithmetic average is %5.3f.' % (float(functools.reduce(add,prices))/float(len(prices))))
    print('The arithmetic Asian call payoff for Strike %5.3f is %5.3f.' \
          % (X, payoff_arithmetric_average_call(prices, X)))
    print('The theoretical result is %5.3f.' % (float(functools.reduce(add,prices))/float(len(prices))-X))
    print('The difference is %5.3f.' \
          % (payoff_arithmetric_average_call(prices, X) - (float(functools.reduce(add,prices))/float(len(prices))-X)))
    print('OK')
