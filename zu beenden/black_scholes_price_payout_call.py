""" ******************************************************
    ...
    Call Option-Preis basieren auf Black&Scholes mit
...
def option_price_european_call_payout(S, X, r, q, sigma, time):
    ...
    call_price = S * math.exp(-q*time)* cum_normal.N(d1) - X * math.exp(-r*time) * cum_normal.N(d2)
    return call_price

if __name__=="__main__":
    print 'Test of Call-Option - european style with continuous dividend yield.'
    S = 100.0
    K = 100.0
    r = 0.1
    sigma = 0.25
    time = 1.0
    q = 0.05
    value = option_price_european_call_payout(S, K, r, q, sigma, time)
