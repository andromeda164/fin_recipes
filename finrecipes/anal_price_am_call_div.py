...
*******************************************************"""
import math
from finrecipes.black_scholes_call import *
import finrecipes.cum_normal_bivariate
import finrecipes.cum_normal
...
def option_price_american_call_one_dividend(S,K,r,sigma,tau,D1,tau1):
    ...
    C = (S-D1*math.exp(-r*tau1)) * finrecipes.cum_normal.N(b1) + (S-D1*math.exp(-r*tau1)) * finrecipes.cum_normal_bivariate.N(a1,-b1,rho) \
      - (K*math.exp(-r*tau))*finrecipes.cum_normal_bivariate.N(a2,-b2,rho) - (K-D1)*math.exp(-r*tau1)*finrecipes.cum_normal.N(b2)
    return C
